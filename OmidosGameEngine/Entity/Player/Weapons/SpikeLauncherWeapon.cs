...
{
    public class SpikeLauncherWeapon : BaseWeapon
    {
        public SpikeLauncherWeapon() :
            base(0.1f)
        {
            numberOfBullets = 2;
            bulletSpeed = 15;
            processorTime = 6;
            accuracy = 15;
            maxDistance = 0.15f * OGE.WorldCamera.Width;

            GunName = GlobalVariables.Data.Weapons[11].Name;
        }

        public override void LoadContent()
        {
            texture = OGE.Content.Load<Texture2D>(@"Graphics\Entities\Bullets\RedSpike");
            baseMask = new HitboxMask(texture.Height, texture.Height, texture.Height / 2, texture.Height / 2);
            TurnetImage = new Image(OGE.Content.Load<Texture2D>(@"Graphics\Entities\Players\Weapons\Spiker"));
        ...
        public override bool GenerateBullet(Vector2 position, float direction, float bonusAccuracy)
        {
            ...
            if (bulletGenerated)
            {
                for (int i = 0; i < numberOfBullets; i++)
                {
                    ...
                    bullet = new SpikeBullet(position, (float)(bulletSpeed * (1 - 0.1 * random.NextDouble())),

...
{
    public class EvaporatorWeapon : BaseWeapon
    {
        public EvaporatorWeapon() :
            base(0.05f)
        {
            numberOfBullets = 2;
            bulletSpeed = 5;
            ...
            GunName = GlobalVariables.Data.Weapons[2].Name;
        }

        public override void LoadContent()
        {
            texture = OGE.Content.Load<Texture2D>(@"Graphics\Entities\Bullets\OrangeVapour");
            baseMask = new HitboxMask(texture.Width, texture.Height, texture.Width / 2, texture.Height / 2);
            TurnetImage = new Image(OGE.Content.Load<Texture2D>(@"Graphics\Entities\Players\Weapons\Evaporator"));
        ...
        public override bool GenerateBullet(Vector2 position, float direction, float bonusAccuracy)
        {
            ...
            if (bulletGenerated)
            {
                for (int i = 0; i < numberOfBullets; i++)
                {
                    ...
                    bullet = new EvaporatorBullet(position, (float)(bulletSpeed * (1 - 0.1 * random.NextDouble())),
